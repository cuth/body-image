	...
	w.BodyImage.prototype = (function () {
		var expandImage = function () {
				var self = this,
					maxWidth = $(w).width(),
					maxHeight = $(w).height(),
					thumbWidth = this.$image.width(),
					thumbHeight = this.$image.height(),
					ratio = thumbWidth / thumbHeight,
					width = maxWidth,
					height = maxWidth / ratio,
					img = new Image(),
					pos = this.$image.offset(),
					scrollLeft = $(w).scrollLeft(),
					scrollTop = $(w).scrollTop(),
					originX, originY, bodyX, bodyY, scale;
				if (height > maxHeight) {
					height = maxHeight;
					width = maxHeight * ratio;
				}
				originX = pos.left + thumbWidth / 2;
				originY = pos.top + thumbHeight / 2;
				scale = width / thumbWidth;
				bodyX = (originX - scrollLeft - maxWidth / 2);
				bodyY = (originY - scrollTop - maxHeight / 2);
				img.src = this.$el.attr('href');
				$(img).load(function () {
					self.$image.attr('src', img.src);
				});
				this.$image.css({ width: thumbWidth, height: thumbHeight });
				this.$body.css({
					width: maxWidth,
					height: maxHeight,
					overflow: 'hidden',
